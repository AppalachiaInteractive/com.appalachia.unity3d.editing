    ...
    public class G_AudioGraph : G_Graph
    {
        ...
        protected override void UpdateGraph()
        {
            ...
            var incrementPerIteration =
                Mathf.FloorToInt(m_audioMonitor.Spectrum.Length / (float) m_resolution);
            ...
            for (var i = 0; i <= (m_resolution - 1); i++)
            {
                ...
                {
                    var value =
                        (m_audioMonitor.dBNormalized(
                             m_audioMonitor.lin2dB(currentValue / incrementPerIteration)
                         ) +
                         m_graphArray[i - 1] +
                         m_graphArray[i - 2]) /
                        3;
                ...
                {
                    m_graphArray[i] = m_audioMonitor.dBNormalized(
                        m_audioMonitor.lin2dB(currentValue / incrementPerIteration)
                    );
                }
            }
            ...
            for (var i = 0; i <= (m_resolution - 1); i++)
            {
                ...
                {
                    m_graphArrayHighestValue[i] = m_audioMonitor.dBNormalized(
                        m_audioMonitor.lin2dB(currentValue / incrementPerIteration)
                    );
                }
            }
