...
#pragma warning restore 0414
        ...
        [SerializeField] private Sprite infoLog;
        [SerializeField] private Sprite warningLog;
        [SerializeField] private Sprite errorLog;

        // Visuals for resize button
        [SerializeField] private Sprite resizeIconAllDirections;
        [SerializeField] private Sprite resizeIconVerticalOnly;
        ...
        [SerializeField] private RectTransform searchbarSlotTop;
        [SerializeField] private RectTransform searchbarSlotBottom;

        [SerializeField] private Image resizeButton;

        [SerializeField] private GameObject snapToBottomButton;

        // Canvas group to modify visibility of the log window
        [SerializeField] private CanvasGroup logWindowCanvasGroup;
...
/*#if ENABLE_INPUT_SYSTEM && !ENABLE_LEGACY_INPUT_MANAGER
        ...
        private void LateUpdate()
        {
            ...
            var queuedLogCount = queuedLogEntries.Count;
            if (queuedLogCount > 0)
            {
            ...
            var logWindowWidth = logWindowTR.rect.width;
            ...
            {
                var scrollPos = logItemsScrollRect.verticalNormalizedPosition;
        ...
        private char OnValidateCommand(string text, int charIndex, char addedChar)
        {
            if (addedChar == '\t') // Autocomplete attempt
            {
                ...
                {
                    var autoCompletedCommand = DebugLogConsole.GetAutoCompleteCommand(text);
        ...
        public void ReceivedLog(string logString, string stackTrace, LogType logType)
        {
            ...
            else
            {
                ...
                if (logLength > maxLogLength)
                {
                    ...
                    var halfMaxLogLength = maxLogLength / 2;
                    ...
                    {
                        // Truncate stackTrace
                        stackTrace = stackTrace.Substring(0, maxLogLength - logString.Length - 12) +
                                     "<truncated>\n";
                    }
                }
            }
                ...
                if (isLogWindowVisible)
                {
                    ...
                    recycledListView.OnCollapsedLogEntryAtIndexUpdated(logEntryIndexInEntriesToShow);
                     ...
                     ((logFilter == DebugLogFilter.All) ||
                      ((logTypeSpriteRepresentation == infoLog) &&
                       ((logFilter & DebugLogFilter.Info) == DebugLogFilter.Info)) ||
                      ((logTypeSpriteRepresentation == warningLog) &&
                       ((logFilter & DebugLogFilter.Warning) == DebugLogFilter.Warning)) ||
                      ((logTypeSpriteRepresentation == errorLog) &&
        ...
        {
            var log = queuedLogEntries[queuedLogEntries.Count - 1];
        ...
        private void RefreshCommandSuggestions(string command)
        {
            ...
            var commandChanged = command != commandInputFieldPrevCommand;
            ...
            if (commandChanged)
            {
                ...
                var prevCommandName = commandInputFieldPrevCommandName;
            ...
            else
            {
                ...
                var suggestionInstancesCount = commandSuggestionInstances.Count;
                var suggestionsCount = matchingCommandSuggestions.Count;
                ...
                for (var i = 0; i < suggestionsCount; i++)
                {
                    ...
                    var suggestedCommand = matchingCommandSuggestions[i];
                ...
                for (var i = visibleCommandSuggestionInstances - 1; i >= suggestionsCount; i--)
                {
        ...
        internal void Resize(PointerEventData eventData)
        {
            ...
            var canvasPivot = canvasTR.pivot;
            var canvasSize = canvasTR.rect.size;
            var anchorMin = logWindowTR.anchorMin;
            ...
            if (enableHorizontalResizing)
            {
                if (resizeFromRight)
                {
                    ...
                    var anchorMax = logWindowTR.anchorMax;
            ...
            // Vertical resizing
            var notchHeight =
                -logWindowTR.sizeDelta.y; // Size of notch screen cutouts at the top of the screen
        ...
        private void CheckScreenCutout()
        {
            ...
            if (safeYMax < (screenHeight - 1)) // 1: a small threshold
            {
                ...
                var cutoutLocalSize = cutoutPercentage * canvasTR.rect.height;
