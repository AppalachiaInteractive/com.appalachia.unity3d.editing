...
namespace Appalachia.Editing.Assets.Organization
{
    ...
    public class OrphanedAsset
    {
        ...
        private static readonly ProfilerMarker _PRF_CreateByPath = new ProfilerMarker(_PRF_PFX + nameof(CreateByPath));
            ...
            where T : class
        {
            using (_PRF_CreateByPath.Auto())
            {
                ...
                asset.analysisResults =
                    AppaTypeFinder.FindTypeByFields<T>(asset.fields.Select(f => f.key));

                return asset;
            }
        }

        private static readonly ProfilerMarker _PRF_UpdateToNewType = new ProfilerMarker(_PRF_PFX + nameof(UpdateToNewType));
