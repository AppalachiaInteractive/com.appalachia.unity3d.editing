...
namespace Appalachia.Editing.Assets.Extensions
{
    public static class AssemblyDefinitionMetadataExtensions
    {
        ...
        public static void Draw(
            ...
            AssemblyDefinitionAssetIssuesContext context,
        ...
        private static readonly ProfilerMarker _PRF_DrawHeader = new ProfilerMarker(_PRF_PFX + nameof(DrawHeader));
        ...
        private static readonly ProfilerMarker _PRF_DrawAssemblyOverview = new ProfilerMarker(_PRF_PFX + nameof(DrawAssemblyOverview));
            ...
            bool useTestFiles)
        {
            using (_PRF_DrawAssemblyOverview.Auto())
            {
                ...
                using (new GUILayout.HorizontalScope())
                {
                    ...
                    (!metadata.HasInvalidAssemblies ? field_issuesNone : field_issuesAsse).Draw(palette.warning);
                    (!metadata.ShouldSortReferences ? field_issuesNone : field_issuesSort).Draw(palette.notable);
                    (metadata.DoesUseGuidReferences ? field_issuesNone : field_issuesGuid).Draw(palette.warning2);
                }
            }
        }

        private static readonly ProfilerMarker _PRF_DrawFields = new ProfilerMarker(_PRF_PFX + nameof(DrawFields));
            ...
            bool useTestFiles)
        {
            using (_PRF_DrawFields.Auto())
            {
                ...
                var field_assembly_current =
                    fieldManager.Get<LabelH4Metadata>("Assembly Name (Current)", f => f.SetPrefixLabelWidth(prefixLabelWidth));
                var field_filename_current = fieldManager.Get<LabelH4Metadata>("File Name (Current)", f => f.SetPrefixLabelWidth(prefixLabelWidth));
                var field_root_namespace_current =
                    fieldManager.Get<LabelH4Metadata>("Root Namespace (Current)", f => f.SetPrefixLabelWidth(prefixLabelWidth));

                var field_assembly_ideal = fieldManager.Get<LabelH4Metadata>("Assembly Name (Ideal)", f => f.SetPrefixLabelWidth(prefixLabelWidth));
                var field_filename_ideal = fieldManager.Get<LabelH4Metadata>("File Name (Ideal)", f => f.SetPrefixLabelWidth(prefixLabelWidth));
                var field_root_namespace_ideal =
                    fieldManager.Get<LabelH4Metadata>("Root Namespace (Ideal)", f => f.SetPrefixLabelWidth(prefixLabelWidth));
        ...
        private static readonly ProfilerMarker _PRF_DrawButtons = new ProfilerMarker(_PRF_PFX + nameof(DrawButtons));
            ...
            bool useTestFiles)
        {
            using (_PRF_DrawButtons.Auto())
            {
                ...
                using (new EditorGUILayout.HorizontalScope())
                {
                    ...
                    var removeInvalidAssembliesButton = fieldManager.Get<MiniButtonMetadata>("Remove Invalid Assemblies");
                    var convertToGuidReferencesButton = fieldManager.Get<MiniButtonMetadata>("Convert To Guids");
                    ...
                    if (sortReferences.Button(metadata.ShouldSortReferences, backgroundColor: palette.notable))
                    {
                        metadata.SortReferences(useTestFiles, true);
                    }

                    if (removeInvalidAssembliesButton.Button(metadata.HasInvalidAssemblies, backgroundColor: palette.warning))
                    {
                        metadata.RemoveInvalidAssemblies(useTestFiles, true);
                    }

                    if (convertToGuidReferencesButton.Button(!metadata.DoesUseGuidReferences, backgroundColor: palette.warning2))
                    {
                        metadata.ConvertToGuidReferences(context.assemblyDefinitionMetadatas, useTestFiles, true);
                    }
                }
            }
        }

        private static readonly ProfilerMarker _PRF_DrawReferences = new ProfilerMarker(_PRF_PFX + nameof(DrawReferences));
